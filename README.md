...
- 서비스 :Controller와 Repository 사이에 존재하는 계층, 처리 업무의 순서를 총괄한다. => Controller는 요청을 받고, 응답을 처리하는 것에만 집중 가능 ! 서비스는 업무 처리 흐름과 트랜잭션 관리 가능! (Rollback) 
- 트랜잭션 : 모두 성공되어야하는 일련의 과정. 중간에 실패 시, 시작 전 상태로 Rollback 
